...
class Scratch3DatabaseBlocks {
     ...
     * menu, e.g. for selecting data and reading meta data. This is designed to work with the project_meta_data
     * extension spec and will not work if project meta data has not been stored
     *** dev - Do we want to display names and ids here?
    ...
    _getProjectIDs() {
        ...
        if(all_projects && all_projects != "undefined") {
            ...
            var project_ids = [];
            all_projects.forEach(function(e) {
                project_ids.push(String(e.id) + ' - ' + String(e.name));
            });
            project_ids.sort();
     ...
     */
     deleteProject(args) {
        ...
        if(all_projects && all_projects != "undefined") {
            ...
            var proj_index = all_projects.findIndex(e => e.id === project_id);
            if(proj_index != -1) {
                all_projects.splice(proj_index, 1);
    ...
    postData(args) {
        ...
        post_data.append('project_id', parseInt(args.PROJECT_ID[0]));
